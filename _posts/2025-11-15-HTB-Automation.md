...
so let’s check the image and see what it is
...
Step 1 — Collects data from the machine

**Step 2 — Encrypts it using AES-CBC**

**Step 3 — Encodes the encrypted data and a command into a DNS TXT request**

**Step 4 — Sends the TXT request to the attacker’s DNS server**
...
2. Attacker Encrypts the Output of the Command
 ...
 4.  The Attacker’s DNS Server Logs All These A‑Record Requests

 5.  DNS Used as a “Covert Channel”

now let’s get all the A records and decrypt them
...
def decrypt_chunks(hex_chunks, key_b64):
    # Convert chunks from hex strings → bytes
    ciphertext = bytes.fromhex("".join(hex_chunks))

    # Decode key (base64 → bytes)
    key = base64.b64decode(key_b64)
